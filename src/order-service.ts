    ...
    app.post('/order/create', (req, res) => {
      try {
        if (!!req.body.CUSTOMER_ID) {
          axios.get(`http://localhost:8081/customer/${req.body.CUSTOMER_ID}`).then(async (result) => {
          let { ACCOUNT, AMOUNT } = result.data
            ...
            let updateBalance = await db.CustomerTable.update(paymentData,{where:{ID:req.body.CUSTOMER_ID}})
            res.send(order)
          }).catch(err=>{
